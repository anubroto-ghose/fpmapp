...
Feature: Audit Trail Logging for Approval Actions
  ...
  So that I can verify compliance and traceability of financial approvals
  ...
  Scenario: Verify audit log creation for approval action with correct details
    When the user performs an approval action on a request with ID "1001"
    ...
    And the audit log entry has the action type "approval"
